...
def plot_rZ(pca, z_s, att, filename, n_examples, net, device, selected_attributes=5):
	...
	for col in range(ncols):
		with torch.no_grad():
			...
			oriX = net( net(oriZ, partition=True), reverse=True)
			...
			recX = net(net(recZ, partition=True), reverse=True)
...
def plot_inverse_umap(filename, stats=None, att=None,
	...
	if inverse_sampling:
		...
		for t_Z_ in tZ.view(4, 25, 3, 64, 64):
			with torch.no_grad():
				t_x_ = net(net(t_Z_, partition=True), reverse=True)
...
def plot_grand_z(grand_zs, names, filename, n_rows_cols=(5, 8), norm='img'):
